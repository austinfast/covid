...
The Howard Center combined four datasets to determine which counties nationwide might have the biggest problem protecting people experiencing homelessness. These 43 counties of highest concern represent 14 states from Hawaii to New York, and include 10 rural counties, 13 suburban counties, 6 urban counties, and 18 of the 50 largest cities in the country. The selected counties possess all the following characteristics:

* High rates of homelessness per capita (according to 2019 CoC population totals computed using an R program [written by a Boston University researcher](https://github.com/tomhbyrne/HUD-CoC-Geography-Crosswalk));
...
* Large number of additional shelter beds needed to accommodate social distancing, as calculated by an April 2020 [University of Pennsylvania/UCLA/Boston University study](https://www.sp2.upenn.edu/the-impact-of-covid-19-on-homelessness/) produced to estimate funding needs for the CARES Act. These researchers predict COVID-19 may hospitalize 4.3% of the nationwide homeless population (21,295 people), with nearly 7,200 of them requiring critical care.

These four variables were selected after consultation with sociologists, demographers and epidemiologists at the U.S. Centers for Disease Control, the University of Pennsylvania, Boston University, the Models of Infection Disease Agent Study (MIDAS) Coordination Center at the University of Pittsburgh, Carnegie Mellon, and Northern Arizona University's Center for Health Equity Research.

Each variable was classified to establish thresholds and select the counties that fall into the risk zones for all four variables. The homeless rate, poverty rate and additional shelter beds were all classified with the [Jenks natural breaks algorithm](https://rdrr.io/github/smh-ej/XploreR/man/mutate_jenks_brks.html), which created five classes based on natural groupings inherent in the data. The break between the first and second classes became the threshold for each variable. The doctors per 100,000 residents rate is measured inversely from the other three variables (higher is better), so the Jenks method was producing unhelpful results. Therefore, we used quantile classification for that variable.
...
We wanted to keep tabs on which of our 43 counties have seen spikes in Covid-19 cases, so the Howard Center turned to datasets updated daily by Johns Hopkins University and Carnegie Mellon University. The Centers for Disease Control assemble case and death data from state and local jurisdictions, but [inconsistent data collection procedures across the country have likely resulted in an undercount](https://www.washingtonpost.com/investigations/cdc-wants-states-to-count-probable-coronavirus-cases-and-deaths-but-most-arent-doing-it/2020/06/07/4aac9a58-9d0a-11ea-b60c-3be060a4f8e1_story.html) of Covid-19's true impact. The CDC tallies cumulative counts as opposed to calculating new cases per day, while the researchers at Johns Hopkins release daily incidence rates, included here. Some dates contain negative figures because of subsequent corrections downward.

Carnegie Mellon recognized the aforementioned inconsistent reporting and devised its own ["COVIDcast" mapping tool](https://covidcast.cmu.edu/?sensor=indicator-combination-nmf_day_doc_fbc_fbs_ght&level=county&region=42003&date=20200611&signalType=value) to create a tracking system they believe is more illustrative of where Covid-19 has greatest impact nationwide. Carnegie Mellon's researchers factor in the percentage of COVID-related outpatient visits with Facebook surveys and Google search trends to create an index of how affected a particular county is by Covid-19 each day. The scale itself is arbitrary, but higher numbers indicate more affected counties. 

Keeping an eye on the day-to-day fluctuations in these three data sources ([CDC](https://public.flourish.studio/visualisation/2895339/), [Johns Hopkins](https://public.flourish.studio/visualisation/2701246/), [Carnegie Mellon](https://public.flourish.studio/visualisation/2701137/)) will help the Howard Center identify Covid-19's effect in these 43 counties and where to focus our reporting efforts throughout this investigation. 
...
The resulting data set contains 43 counties of highest concern. Visit the [ArcGIS map here](https://arcg.is/0Lfm1q) to visualize where they fall on the map. 
